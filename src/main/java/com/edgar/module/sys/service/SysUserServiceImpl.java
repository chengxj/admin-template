...
public class SysUserServiceImpl implements SysUserService {
        ...
        @Override
        public Pagination<SysUser> pagination(QueryExample example, int page, int pageSize) {
        ...
        public int updatePassword(PasswordCommand command) {
                ...
                if (!command.getNewpassword().equals(command.getRetypepassword())) {
                        throw ExceptionFactory.inValidParameter("msg.error.twoPasswordNotEquals");
                }
                SysUser oldUser = sysUserDao.get(command.getUserId());
                String oldPassword = command.getOldpassword();
                
                String encryptPassword = PasswordHelper.getEncryptPassword(oldPassword, oldUser);
                if (!encryptPassword.equals(oldUser.getPassword())) {
                        throw ExceptionFactory.inValidParameter("msg.error.passwordInit.error");
                }
