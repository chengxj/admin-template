...
public class LoginUserUtils {
        ...
        public static final void getPermission(LoginUser loginUser) {
                ...
                CrudRepository<Integer, SysRoleResource> sysRoleResourceDao = ServiceLookup
                                .getBean("sysRoleResourceDao", CrudRepository.class);
                CrudRepository<Integer, SysResource> sysResourceDao = ServiceLookup.getBean(
                                "sysResourceDao", CrudRepository.class);
                for (SysRole role : roles) {
                        QueryExample example = QueryExample.newInstance();
                        example.equalsTo("roleId", role.getRoleId());
                        List<SysRoleResource> sysRoleMenus = sysRoleResourceDao.query(example);
                        for (SysRoleResource sysRoleResource : sysRoleMenus) {
                                permissions.add(sysResourceDao.get(sysRoleResource.getResourceId())
                                                .getPermission());
                        }
                }
